...
// Guarded on purpose: the Supabase CDN script sometimes hasn't loaded
// (or fails to load) by the time this file runs — slow mobile data, an
// ad/tracker blocker, a school/corporate network filter, etc. This is
// far more common on some devices/networks than others, which is why
// the symptom was "some devices work, some don't." When that happened,
// `window.supabase.createClient(...)` used to throw right here, and
// because that's a top-level statement, the throw stopped every other
// top-level statement below it from running — including the `let`
// declarations for currentUser/isGuestMode/authMode/cloudSyncTimeout.
// Those variables were still *declared* (hoisted) but never
// initialized, so the moment enterGuestMode() tried to do
// `currentUser = null;`, it threw "Cannot access 'currentUser' before
// initialization" and aborted before bootAppUI() could ever run —
// silently breaking the guest button, even though guest mode has
// nothing to do with Supabase. Falling back to sb = null keeps every
// other top-level statement (and thus guest mode) working normally;
// only the actual cloud-sync/sign-in calls need Supabase.
let sb = null;

// Tracks whether we've already wired sb.auth.onAuthStateChange, so
// initSupabaseClient() can attach it the moment sb becomes available
// even if that happens after the very first attempt (see below).
let authListenerAttached = false;

// Attempts to (re)create the Supabase client if it isn't already set.
// This used to be a single top-level try/catch that ran once when this
// file first executed. The problem: if window.supabase (from the CDN
// script) hadn't finished attaching by that exact instant — slow
// network, cache miss, script load order, even just page-load timing
// that varies run to run on the same device — sb was set to null and
// nothing ever tried again. Guest mode kept working (it never needed
// sb), but handleAuthSubmit()'s `if (!sb) { ...error...; return; }`
// guard meant login/signup was then permanently treated as
// unavailable for the rest of the session, even once Supabase finished
// loading a moment later. Calling this function again (e.g. right
// before a login attempt) lets a late-arriving Supabase script recover
// on its own instead of being latched into a false "unavailable" state.
...
let authMode = 'signin'; // 'signin' | 'signup' | 'reset-request' | 'reset-confirm'
...
function initAuthListeners() {
    // sb can be null if Supabase hadn't finished loading yet (see
    // initSupabaseClient() above) — attachAuthStateListener() no-ops
    // in that case rather than throwing, and initSupabaseClient() will
    // attach it later on its own if a retry (e.g. before login)
    // succeeds.
    attachAuthStateListener();
...
function attachAuthStateListener() {
    ...
    sb.auth.onAuthStateChange((event, session) => {
        if (event === 'PASSWORD_RECOVERY') {
            // Person clicked the "reset your password" link from their email —
            // Supabase has already verified them, just needs a new password.
...
function renderAuthMode() {
    ...
    if (authMode === 'signup') {
        ...
        subtitle.textContent = "We'll email you a link to set a new one.";
...
async function handleAuthSubmit() {
    // Retry initializing Supabase here rather than trusting whatever sb
    // was (or wasn't) at page load — see initSupabaseClient() above.
    if (!sb) initSupabaseClient();

    if (!sb) {
        showAuthMessage('Unable to reach the sign-in service right now. Check your connection and try again, or continue as a guest.', 'error');
        return;
    }
    ...
    // ---- Requesting a reset link (only needs an email) ----
    if (authMode === 'reset-request') {
        ...
        try {
            ...
            if (error) throw error;
            showAuthMessage('Check your email for a reset link.', 'success');
        } catch (e) {
...
async function handleSignOut() {
    ...
    } catch (e) { /* still clear local UI state below regardless */ }
    currentUser = null;
    authMode = 'signin';
    renderAuthMode();
    clearGreeting();
